    ...
    <div className="course-list">
      <ul>
        {Object.keys(courses).map(courseKey => (
          <li key={courseKey}>
            <strong>{courses[courseKey].title}</strong> - {courses[courseKey].meets}
          </li>
        ))}
      </ul>
    </div>
...
const courses = ({courses}) => (
  <div>
    { Object.entries(courses).map(([id, course]) => courseinfo(course)) }
  </div>
);
...
const App = () => {
  ...
  return (
    ...
    <div>
      <p>{schedule.title} </p>
      <p>{schedule.courses.term} CS {schedule.courses.number}: {schedule.courses.title}</p>
     <p>{Object.entries(schedule.courses).map(([id, user]) => <courseinfo key={id} user={user} />) }</p>
  </div>
